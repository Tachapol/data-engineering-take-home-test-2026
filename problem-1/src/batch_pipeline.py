...
def run_etl(execution_date):
    ...
    if daily_rev.rdd.isEmpty():
        ...
        # Use psycopg2 to insert a typed DATE value (avoids Spark -> JDBC literal casting issues)
        import psycopg2
        from datetime import datetime
        date_obj = datetime.strptime(execution_date, "%Y-%m-%d").date()
        conn = psycopg2.connect(host='postgresql', dbname='postgres', user='postgres', password='postgres')
        try:
            cur = conn.cursor()
            cur.execute(
                """
                INSERT INTO daily_revenue (date, total_revenue, total_orders, total_quantity, avg_order_value)
                VALUES (%s, %s, %s, %s, %s)
                ON CONFLICT (date) DO NOTHING
                """,
                (date_obj, 0.0, 0, 0, 0.0)
            )
            conn.commit()
            cur.close()
            print(f"Inserted zero-summary row for {execution_date} via psycopg2")
        finally:
            conn.close()
    else:
